...
export default function AiAnalyzer({ auth }) {
    ...
    // The model sometimes wraps the JSON in prose or code fences ("Sure! Here are 5 more
    // questions:\n```json\n[...]\n```"). Strip fences, then pull out the first [...] block
    // instead of assuming the whole reply is clean JSON.
    ...
    const FLASHCARD_INDEX_KEY = 'studyshare_flashcards_index';

    const loadSavedSets = () => {
        try {
            const raw = localStorage.getItem(FLASHCARD_INDEX_KEY);
            const index = raw ? JSON.parse(raw) : [];
            setSavedSets(Array.isArray(index) ? index : []);
        } catch (e) {
            setSavedSets([]);
        }
    };
    ...
    const handleDeleteSet = (set) => {
        ...
        const updated = savedSets.filter(s => s.key !== set.key);
        localStorage.setItem(FLASHCARD_INDEX_KEY, JSON.stringify(updated));
        setSavedSets(updated);
        if (viewingSet?.key === set.key) setViewingSet(null);
    };
    ...
    const handleGenerateMoreQuiz = async () => {
        ...
        setQuizGenError('');
        try {
            const documentText = result.document_text || '';
            const existingQuestions = result.analysis.quiz_questions.map(q => q.question).join(', ');
            ...
            const res = await fetch('/student/ai-analyzer/chat', {
                ...
                credentials: 'same-origin',
                body: JSON.stringify({
                    message: 'Generate 5 NEW multiple choice quiz questions about this document. These questions already exist so don\'t repeat them: ' + existingQuestions + '. Return ONLY a raw JSON array, no commentary, no markdown fences, in this exact shape: [{"question":"...","options":["A) ...","B) ...","C) ...","D) ..."],"answer":"A) ...","explanation":"..."}]. Document: ' + documentText.slice(0, 3000),
                    history: [],
                }),
            });
            ...
            if (!data.reply) throw new Error('No reply received from the AI.');

            const newQuestions = extractJsonArray(data.reply);
            if (newQuestions.length === 0) throw new Error('AI returned zero new questions — try again.');

            setResult(prev => ({
                ...prev,
                analysis: {
                    ...prev.analysis,
                    quiz_questions: [...prev.analysis.quiz_questions, ...newQuestions]
                }
            }));
        } catch (e) {
            console.error('Failed to generate more quiz questions', e);
            setQuizGenError(e.message || 'Could not generate more questions. Please try again.');
        }
    ...
    const handleGenerateMoreFlashcards = async () => {
        ...
        try {
            const documentText = result.document_text || '';
            const existingTerms = result.analysis.flashcards.map(f => f.term).join(', ');
            ...
            const res = await fetch('/student/ai-analyzer/chat', {
                ...
                credentials: 'same-origin',
                body: JSON.stringify({
                    message: 'Generate 6 NEW flashcards about this document. These terms already exist so don\'t repeat them: ' + existingTerms + '. Return ONLY a raw JSON array, no commentary, no markdown fences, in this exact shape: [{"term":"...","definition":"..."}]. Document: ' + documentText.slice(0, 3000),
                    history: [],
                }),
            });
            ...
            const newCards = extractJsonArray(data.reply);
            if (newCards.length === 0) throw new Error('AI returned zero new flashcards — try again.');

            setResult(prev => ({
                ...prev,
                analysis: {
                    ...prev.analysis,
                    flashcards: [...prev.analysis.flashcards, ...newCards]
                }
            }));
        } catch (e) {
            console.error('Failed to generate more flashcards', e);
            setFlashcardGenError(e.message || 'Could not generate more flashcards. Please try again.');
        }
    ...
    const handleSaveFlashcards = () => {
        if (!result) return;
        const title = result.resource?.title || 'Untitled Document';
        const subject = result.resource?.subject || '';
        const key = 'studyshare_flashcards_' + title.replace(/\s+/g, '_').replace(/[^\w\-]/g, '');

        try {
            localStorage.setItem(key, JSON.stringify(result.analysis.flashcards));

            const raw = localStorage.getItem(FLASHCARD_INDEX_KEY);
            const index = raw ? JSON.parse(raw) : [];
            const withoutThisSet = (Array.isArray(index) ? index : []).filter(item => item.key !== key);
            withoutThisSet.unshift({
                key,
                title,
                subject,
                count: result.analysis.flashcards.length,
                savedAt: new Date().toISOString(),
            });
            localStorage.setItem(FLASHCARD_INDEX_KEY, JSON.stringify(withoutThisSet));
            setSavedSets(withoutThisSet);

            setSavedFlashcards(true);
            setTimeout(() => setSavedFlashcards(false), 3000);
        } catch (e) {
            setError('Could not save flashcards — your browser storage may be full or disabled.');
        }
    };
    ...
    const handleSendChat = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            setChatMessages(prev => [...prev, { role: 'assistant', content: data.error ? 'Sorry, I encountered an error. Please try again.' : data.reply }]);
        } catch (e) {
    ...
    const cardClass = `border-2 border-b-[8px] border-black rounded-3xl p-5 ${cardBg}`;
    ...
    return (
        <MainLayout auth={auth}>
            ...
            <div className="flex gap-2 sm:gap-3 mb-6 font-sans-custom">
                ...
                <button onClick={() => { setActiveSection('library'); setViewingSet(null); loadSavedSets(); }} className={sectionTabClass('library')}>
                    📚 My Flashcards{savedSets.length > 0 ? ` (${savedSets.length})` : ''}
                </button>
            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                            <div className="flex flex-col gap-4">
                                ...
                                {activeTab === 'quiz' && (
                                    <div className={cardClass}>
                                        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                                            <h2 className={`font-unique-bold text-lg uppercase tracking-wide ${textHeader}`}>❓ Quiz Questions</h2>
                                            <button
                                                onClick={handleGenerateMoreQuiz}
                                                disabled={generatingQuiz}
                                                className="bg-blue-400 text-black uppercase font-black tracking-wider border-2 border-black disabled:opacity-50 text-xs px-3 py-1.5 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none flex items-center gap-1.5"
                                            >
                                                {generatingQuiz ? (
                                                    <>
                                        ...
                                        {quizGenError && (
                                            <div className="mb-4 p-3 bg-red-300 border-2 border-black rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                                <p className="text-black text-xs font-black uppercase tracking-wide">⚠️ {quizGenError}</p>
                                            </div>
                                        )}
                                        <div className="flex flex-col gap-6">
                                            {result.analysis.quiz_questions.map((q, i) => (<QuizQuestion key={i} question={q} index={i} dark={dark} inputBg={inputBg} textHeader={textHeader} textMuted={textMuted} />))}
                                        </div>
                                    </div>
                                )}
                                {activeTab === 'flashcards' && (
                                    <div className={cardClass}>
                                        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                                            <h2 className={`font-unique-bold text-lg uppercase tracking-wide ${textHeader}`}>🃏 Flashcards ({result.analysis.flashcards.length})</h2>
                                            <div className="flex gap-2">
                                                <button
                                                    onClick={handleGenerateMoreFlashcards}
                                                    disabled={generatingFlashcards}
                                                    className="bg-purple-400 text-black uppercase font-black tracking-wider border-2 border-black disabled:opacity-50 text-xs px-3 py-1.5 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none flex items-center gap-1.5"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="bg-emerald-400 text-black uppercase font-black tracking-wider border-2 border-black text-xs px-3 py-1.5 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none"
                                                >
                                                    💾 Save All
                                                </button>
                                            </div>
                                        ...
                                        {flashcardGenError && (
                                            <div className="mb-4 p-3 bg-red-300 border-2 border-black rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                                <p className="text-black text-xs font-black uppercase tracking-wide">⚠️ {flashcardGenError}</p>
                                            </div>
                                        )}

                                        {savedFlashcards && (
                                            <div className="mb-4 p-3 bg-emerald-100 border-2 border-black rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                                                <p className="text-black text-xs font-bold uppercase tracking-wide">✅ Saved to "My Flashcards" — find it anytime from the tab at the top.</p>
                                            </div>
                                        )}

                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {result.analysis.flashcards.map((card, i) => (
                                                <Flashcard key={i} card={card} index={i} dark={dark} inputBg={inputBg} textHeader={textHeader} />
                                            ))}
                                        </div>
                                    </div>
                                )}
                                    ...
                                    {docChatMessages.map((msg, i) => (
                                        <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`flex items-start gap-2.5 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                                ...
                                                <div className={`px-3.5 py-2.5 rounded-2xl border-2 border-black font-bold text-xs leading-relaxed shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${msg.role === 'user' ? 'bg-blue-300 text-black' : inputBg}`}>
                        ...
                        {chatMessages.map((msg, i) => (
                            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`flex items-start gap-2.5 max-w-[85%] sm:max-w-xl ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                    ...
                                    <div className={`px-4 py-3 rounded-2xl border-2 border-black font-bold text-xs leading-relaxed shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${msg.role === 'user' ? 'bg-blue-300 text-black' : inputBg}`}>
                                        {msg.content.split('\n').map((line, j) => (<p key={j} className={j > 0 ? 'mt-1' : ''}>{line}</p>))}
                                    </div>
                                </div>
                            </div>
            ...
            {activeSection === 'library' && (
                <div className="font-sans-custom">
                    {!viewingSet && (
                        <div className={`border-2 border-b-[8px] border-black rounded-3xl p-5 sm:p-6 ${cardBg}`}>
                            <h2 className={`font-unique-bold text-xl uppercase tracking-wide mb-1 ${textHeader}`}>📚 My Flashcards</h2>
                            <p className={`text-xs font-bold uppercase tracking-widest ${textMuted} mb-5`}>Sets saved from the PDF Analyzer, stored right in this browser</p>

                            {savedSets.length === 0 ? (
                                <div className={`text-center py-10 border-2 border-dashed border-black rounded-2xl ${inputBg}`}>
                                    <p className="text-4xl mb-2">🗃️</p>
                                    <p className={`font-unique-bold text-sm uppercase ${textHeader}`}>No saved flashcard sets yet</p>
                                    <p className={`text-xs font-bold uppercase mt-1 ${textMuted}`}>Analyze a document, open the Cards tab, and hit "Save All"</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {savedSets.map(set => (
                                        <div key={set.key} className={`border-2 border-black rounded-2xl p-4 flex flex-col gap-3 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] ${inputBg}`}>
                                            <div>
                                                <p className={`font-unique-bold text-sm uppercase leading-snug ${textHeader}`}>{set.title}</p>
                                                {set.subject && (
                                                    <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${textMuted}`}>{set.subject}</p>
                                                )}
                                                <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mt-2">🃏 {set.count} cards</p>
                                                <p className={`text-[10px] font-bold uppercase mt-1 ${textMuted}`}>
                                                    Saved {new Date(set.savedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                                                </p>
                                            </div>
                                            <div className="flex gap-2 mt-auto">
                                                <button
                                                    onClick={() => handleViewSet(set)}
                                                    className="flex-1 bg-blue-400 text-black uppercase font-black tracking-wider border-2 border-black text-xs px-3 py-2 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none"
                                                >
                                                    📖 Study
                                                </button>
                                                <button
                                                    onClick={() => handleDeleteSet(set)}
                                                    className="bg-red-400 text-black uppercase font-black tracking-wider border-2 border-black text-xs px-3 py-2 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none"
                                                >
                                                    🗑️
                                                </button>
                                            </div>
                            ...
                            <div className="flex items-center justify-between mb-5 flex-wrap gap-2">
                                <div>
                                    <button
                                        ...
                                        className={`text-[10px] font-black uppercase tracking-widest mb-2 ${textMuted}`}
                                    >
                                        ← Back to all sets
                                    </button>
                                    <h2 className={`font-unique-bold text-xl uppercase tracking-wide ${textHeader}`}>{viewingSet.title}</h2>
                                    <p className={`text-[10px] font-bold uppercase tracking-widest ${textMuted}`}>{viewingSet.cards.length} cards</p>
                                </div>
                                <button
                                    onClick={() => handleDeleteSet(viewingSet)}
                                    className="bg-red-400 text-black uppercase font-black tracking-wider border-2 border-black text-xs px-3 py-1.5 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none"
                                >
                                    🗑️ Delete Set
                                </button>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {viewingSet.cards.map((card, i) => (
                                    <Flashcard key={i} card={card} index={i} dark={dark} inputBg={inputBg} textHeader={textHeader} />
                                ))}
                            </div>
                        </div>
...
function QuizQuestion({ question, index, dark, inputBg, textHeader, textMuted }) {
    const [selected, setSelected] = useState(null);
    const [revealed, setRevealed] = useState(false);

    return (
        <div className={`p-4 rounded-xl border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${inputBg}`}>
            <p className={`font-bold text-xs mb-3 ${textHeader}`}>
                <span className="bg-blue-400 text-black border border-black text-xs px-2 py-0.5 rounded-lg mr-2 font-black uppercase">Q{index + 1}</span>
                {question.question}
            </p>
            <div className="flex flex-col gap-2 mb-3">
                {question.options.map((option, i) => (
                    <button
                        ...
                        className={'text-left px-3 py-2 rounded-xl text-xs font-bold transition border-2 border-black shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] ' + (
                            selected === option
                                ? (revealed
                                    ? (option === question.answer ? 'bg-emerald-300 text-black' : 'bg-red-300 text-black')
                                    : 'bg-blue-300 text-black')
                                : (revealed && option === question.answer
                                    ? 'bg-emerald-300 text-black'
                                    : (dark ? 'bg-stone-800 text-stone-300' : 'bg-white text-stone-700'))
                        )}
                    >
                        {option}
                    </button>
            ...
            {!revealed ? (
                <button
                    ...
                    disabled={!selected}
                    className="bg-blue-400 text-black uppercase font-black tracking-wider border-2 border-black disabled:opacity-40 text-xs px-3 py-1.5 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none"
                >
                    Check Answer
                </button>
            ) : (
                <div className={`mt-2 p-3 rounded-xl border-2 border-black ${dark ? 'bg-stone-800' : 'bg-white'}`}>
                    <p className={'text-xs font-black uppercase tracking-wide ' + (selected === question.answer ? 'text-emerald-500' : 'text-red-500')}>
                        {selected === question.answer ? '✅ Correct!' : '❌ Incorrect'}
                    </p>
                    <p className={`text-xs mt-1 font-bold ${textHeader}`}>
                        Answer: {question.answer}
                    </p>
                    <p className={`text-xs mt-1 font-bold ${textMuted}`}>{question.explanation}</p>
                </div>
            )}
        </div>
    );
...
function Flashcard({ card, index, dark, inputBg, textHeader }) {
    const [flipped, setFlipped] = useState(false);

    return (
        <div
            onClick={() => setFlipped(!flipped)}
            style={{ perspective: '1000px', cursor: 'pointer', minHeight: '140px' }}
        >
            <div style={{
                position: 'relative',
                width: '100%',
                ...
                {/* Front */}
                <div style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', position: 'absolute', width: '100%', height: '100%' }}
                    className={`rounded-xl border-2 border-black p-4 flex flex-col justify-between shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${inputBg}`}
                >
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg border border-black bg-blue-400 text-black">
                            Term #{index + 1}
                        </span>
                        <span className={`text-[10px] font-bold uppercase ${dark ? 'text-stone-500' : 'text-stone-400'}`}>Click to flip</span>
                    </div>
                    <p className={`font-unique-bold text-center text-sm ${textHeader}`}>{card.term}</p>
                    <div className="flex justify-center">
                        <span className={`text-[10px] font-bold uppercase ${dark ? 'text-stone-500' : 'text-stone-400'}`}>🔄 Tap to see definition</span>
                    </div>
                </div>

                {/* Back */}
                <div style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', position: 'absolute', width: '100%', height: '100%', transform: 'rotateY(180deg)' }}
                    className="rounded-xl border-2 border-black p-4 flex flex-col justify-between bg-blue-400 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                >
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg border border-black bg-white text-black">
                            Definition
                        </span>
                        <span className="text-[10px] font-bold uppercase text-black/60">Click to flip back</span>
                    </div>
                    <p className="text-black font-bold text-xs text-center leading-relaxed">{card.definition}</p>
                    <div className="flex justify-center">
                        <span className="text-[10px] font-bold uppercase text-black/60">🔄 Tap to see term</span>
                    </div>
                </div>
            </div>
        </div>
